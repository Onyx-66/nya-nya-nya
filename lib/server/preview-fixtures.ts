import seriesFixtures from "../fixtures/mangadex-series.json";
import { defaultCommercialSettings } from "../commercial-settings";

export const PREVIEW_FIXTURE_VERSION = "mangadex-browse-49.22.21";
const prefix = "pv4921";
...
export function previewFixtureStatements(db: D1Database, adminEmails: string[], pageHashes: string[]) {
  ...
  const people = ["Mina Vale", "Kai Rowan", "Nora Finch", "Arlo Chen", "Lina Ash", "Rin Frost", "Sora Lane", "Theo Moon", "Yuna Reed", "Owen Star", "Ari Dawn", "Nova Wren"];
  ...
  people.forEach((name, index) => {
    ...
    add("INSERT OR IGNORE INTO user_profiles (user_id,username,normalized_username,bio,profile_visibility,banner_key) VALUES (?,?,?,?,'PUBLIC',?)", id, username, username, "Fictional reader for preview testing. This is not a MangaDex user account.", seriesFixtures[index % seriesFixtures.length].cover);
  });
  ["Moonlit Pages", "Blue Lantern Lab", "Golden Leaf Studio"].forEach((name, index) => {
    const team = `${prefix}-team-${index}`;
    add("INSERT OR IGNORE INTO teams (id,public_ref,slug,name,created_by_user_id,description,logo_key,banner_key,verification_status) VALUES (?,?,?,?,?,?,?,?,'VERIFIED')", team, `TM-PV4921-${index}`, `preview-${slugify(name)}`, `${name} · Preview`, manager, "Fictional team for upload and analytics testing.", seriesFixtures[index].cover, seriesFixtures[index].cover);
  ...
  seriesFixtures.forEach((fixture, index) => {
    ...
    add(`INSERT OR IGNORE INTO series (id,public_ref,slug,title,synopsis,type,status,origin_country,original_language,reading_direction,publication_year,access_type,cover_key,banner_key,slider_key,rating_tenths,follower_count,view_count,rights_status,is_published,created_at,updated_at)
      ...
      sid, `SR-PV4921-${index}`, slug, fixture.title,
    ...
    if (chapters) {
      add(`WITH RECURSIVE nums(n) AS (SELECT 1 UNION ALL SELECT n+1 FROM nums WHERE n < ?)
        ...
        SELECT ? || '-c' || n, ? || '-' || n, ?, ?, ?, 'chapter-' || n, CAST(n AS TEXT), 'Preview chapter ' || n, 'en', 'VERTICAL', 'PUBLISHED',
          ...
          3, datetime('now', '-' || (? - n) || ' hours'), 'Original placeholder pages for testing only.', ? FROM nums WHERE NOT EXISTS (SELECT 1 FROM chapters existing WHERE existing.id=? || '-c' || nums.n)`,
        chapters, sid, `CH-PV4921-${index}`, sid, team, manager, paid ? 1 : 0, chapters, paid ? 1 : 0, chapters, chapters, fixture.cover, sid);
      for (let page = 0; page < 3; page++) add("INSERT OR IGNORE INTO chapter_pages (id,chapter_id,page_index,object_key,width,height,sha256,processing_status) SELECT id || ?,id,?,?,900,1350,?,'READY' FROM chapters WHERE series_id=?", `-p${page}`, page, `preview/49.22.21/page-${page + 1}.svg`, pageHashes[page], sid);
    }
    if (index < 5) {
      add("INSERT OR IGNORE INTO homepage_sliders (id,series_id,title,category_label,short_description,destination_url,image_key,is_active,sort_order,created_by_user_id) SELECT ?,?,?,'PREVIEW',?,?,?,CASE WHEN (SELECT COUNT(*) FROM homepage_sliders WHERE is_active=1)<9 THEN 1 ELSE 0 END,?,?", `${sid}-slider`, sid, fixture.title, "MangaDex reference · original test chapters", `/title/${slug}`, fixture.cover, index + 100, manager);
...
export async function ensurePreviewFixtures(env: PreviewEnv) {
  ...
  initialization = (async () => {
    ...
    for (let page = 1; page <= 3; page++) {
      ...
      await bucket.put(`preview/49.22.21/page-${page}.svg`, content, { httpMetadata: { contentType: "image/svg+xml" } });
    }
