    ...
    <div className="account-security-workspace">
      ...
      <section className="security-method-card" aria-labelledby="security-totp-title">
        ...
        {!status?.totpEnrolled && !setup ? (
          <button className="button button-primary" type="button" onClick={() => void beginTotp()} disabled={Boolean(busy)}>{busy === "totp-begin" ? <DotsRing size={18} /> : <Key size={18} />} Enable authenticator</button>
        ) : null}
        {setup ? (
          <div className="totp-setup-panel">
            <div className="totp-setup-visual"><div className="totp-qr-frame">{qrDataUrl ? <img src={qrDataUrl} alt="QR code for authenticator enrollment" /> : <DotsRing size={28} label="Generating QR code" />}</div><small>Scan with Google Authenticator, 1Password, Microsoft Authenticator, or another TOTP app.</small></div>
            <div className="totp-setup-details"><div><span className="security-label">Manual setup key</span><code>{setup.secret}</code></div><div><span className="security-label">Authenticator code</span><input className="security-code-input" inputMode="numeric" autoComplete="one-time-code" maxLength={6} value={totpCode} onChange={(event) => setTotpCode(event.target.value.replace(/\D/gu, "").slice(0, 6))} placeholder="000000" /></div><button className="button button-primary" type="button" onClick={() => void verifyTotp()} disabled={busy === "totp-verify" || totpCode.length !== 6}>{busy === "totp-verify" ? <DotsRing size={18} /> : <Check size={18} />} Confirm and enable</button><button className="button button-quiet" type="button" onClick={() => setSetup(null)} disabled={Boolean(busy)}>Cancel</button></div>
          </div>
        ) : null}
        {status?.totpEnrolled ? (
