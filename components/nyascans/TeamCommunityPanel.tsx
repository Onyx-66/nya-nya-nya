    ...
    <section className="team-community-workspace">
      ...
      <details className="team-create-forum" open={!data.teams.length}>
        ...
        <div><p>Verification method: an administrator opens the public proof link and validates your statement before ownership and publishing rights become active.</p>
          ...
          <label><span>Ownership proof link</span><select value={create.proofUrl} onChange={(event) => setCreate((current) => ({ ...current, proofUrl: event.target.value }))}><option value="">Select a completed link</option>{create.links.filter((link) => link.url).map((link) => <option key={link.url} value={link.url}>{link.label} · {link.url}</option>)}</select></label>
